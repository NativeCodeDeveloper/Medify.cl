...
const FOOTER_LINKS = [
  ...
  },
  {
    ...
    items: [
      { label: "Psicología", href: "/marketplace" },
      { label: "Kinesiología", href: "/marketplace" },
      { label: "Nutrición", href: "/marketplace" },
      { label: "Medicina General", href: "/marketplace" },
    ],
  },
  {
    heading: "Legal",
...
export default function Footer() {
  return (
    <footer
      className={`${inter.className} bg-[#f5f5f7] border-t border-[#d2d2d7]`}
      style={{ fontSize: "12px" }}
    >
      {/* Link grid */}
              ...
              <ul className="space-y-2.5">
                {col.items.map((item) => (
                  <li key={item.label}>
                    <Link
                      href={item.href}
                      className="text-[#6e6e73] hover:text-[#1d1d1f] transition-colors duration-150"
                      style={{ fontSize: "12px" }}
                    >
                      {item.label}
                    </Link>
        ...
        <div className="border-t border-[#d2d2d7] pt-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <a
                href="https://www.instagram.com/medify.chile"
                target="_blank"
                rel="noreferrer"
                className="text-[#6e6e73] hover:text-[#1d1d1f] transition-colors duration-150"
